...
export default function AlbumCreateSelect({ title: initialTitle = '', photos: initialPhotos = [] }: Props) {
  ...
  const onUploadChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Auto reload page to display newly uploaded photos/videos
  ...
  const submitCreate = useCallback(() => {
    ...
    router.post('/albums', { title, photo_ids }, { preserveScroll: true });
  }, [title, selectedIds]);

  return (
    <>
      <Head title="Chọn ảnh để tạo album" />
      <div className="favourites-page has-selection">
        ...
        <header className="favourites-header" style={{position:'sticky', top:0, zIndex:20}}>
          <div className="favourites-header-content">
            <Link href="/albums" className="back-button" aria-label="Quay lại album">
            ...
            <div className="header-actions">
              ...
              <button className="action-done" onClick={submitCreate} title="Xong">
                Xong
              </button>
            </div>
          </div>
        ...
        {/* Grid reuse styles from photos/favourites */}
